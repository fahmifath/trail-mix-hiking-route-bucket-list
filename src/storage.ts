...
export function load(): LoadResult {
  try {
    ...
    if (!Array.isArray(parsed)) {
      return { status: "error", message: "Saved data has an unexpected format. Starting fresh." };
    }
    const valid: Trail[] = [];
    let rejected = 0;
    ...
    return { status: "error", message: `Could not read saved data: ${msg}` };
  }
}
...
export function save(items: Trail[]): SaveResult {
  try {
    ...
    if (e instanceof DOMException && e.name === "QuotaExceededError") {
      return { ok: false, message: "Storage quota exceeded. Remove some trails to save." };
    }
    const msg = e instanceof Error ? e.message : "Unknown error";
    return { ok: false, message: `Could not save trails: ${msg}` };
  }
}
